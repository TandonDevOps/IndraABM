...
from APIServer.model_process import (createModelProcess,
                                     Message,
                                     CommunicationType,
                                     createNewModel)
...
class ModelProcessAttrs:
    ...
    """
        New model is being created, once the endpoint is done
        we call spawn_model within ModelManager
    """
...
class ModelManager:
    ...
    def __init__(self):
        ...
        """
            Max number of process to run at parallel
            Can be used to check before caching a process
        """
        self.maxSize = cpu_count() * 5 + 1
        self.processes = LRU(self.maxSize)
    ...
    """
        First step to occur after model is initialized
        When we start the server we don't have
        child processes until we get requests
        model_id is used as a username for that model to identify it
    """
    def spawn_model(self, model_id=None, payload=None, model_name=None,
                    isTest=False):
        if(len(self.processes.items()) == self.maxSize):
            self.terminate_model()
        """
            We use pipe to communicate between parent and child
            Child process runs the actual model
        """
        parent_conn, child_conn = Pipe()
        # Each model runs in its own process
        if model_name is None:
            new_process = Process(target=createModelProcess,
                                  args=(child_conn, model_id, payload, isTest))
        else:
            new_process = Process(target=createNewModel,
                                  args=(child_conn, model_name))
        mp = ModelProcessAttrs(new_process, parent_conn,
                               model_id if not isTest else TEST_MODEL_ID)
        new_process.start()
    ...
    def get_process(self, exec_key):
        if(exec_key not in self.processes):
    ...
    def get_agent(self, exec_key, agent_name):
        modelProcess = self.get_process(exec_key)
        message = Message(CommunicationType.AGENT_INFO,
                          {'agent_name': agent_name})
        modelProcess.parent_conn.send(message)
        agent = modelProcess.parent_conn.recv()
        return agent

    def create_group(self, exec_key, jrep, group_color,
                     group_num_of_members, group_name):
        modelProcess = self.get_process(exec_key)
        message = Message(CommunicationType.CREATE_GROUP,
                          {'jrep': jrep,
