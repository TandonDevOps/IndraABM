...
def listenForMessages(conn, model):
    """
        The process then goes into an infinite loop listening on the pipe
    """
    while True:
        message = conn.recv()
        if message.type == CommunicationType.RUN_MODEL:
            ...
            new_group = create_group(message.data['jrep'],
                                     message.data['group_color'],
                                     message.data['group_num_of_members'],
...
"""
    ...
    The process then waits on the parent process
    to send messages with a communication type
...
def createModelProcess(conn, model_id, payload, is_test):
    if(is_test):
        ...
        model = create_model(model_id, payload)
    """
        The first time a process is created,
        send it back through the pipe
    """
    conn.send(model)
