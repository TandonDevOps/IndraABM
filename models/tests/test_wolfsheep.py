...
class WolfSheepTestCase(TestCase):
    ...
    def test_wolf_action(self):
        ...
        if time_to_repro == 1:
            self.assertEqual(
                self.wolf.get_attr(TIME_TO_REPRODUCE), WOLF_TIME_TO_REPRO
            )
        else:
            self.assertEqual(
                self.wolf.get_attr(TIME_TO_REPRODUCE), time_to_repro - 1
            )
    ...
    def test_sheep_action(self):
        ...
        if time_to_repro == 1:
            self.assertEqual(
                self.sheep.get_attr(TIME_TO_REPRODUCE), SHEEP_TIME_TO_REPRO
            )
        else:
            self.assertEqual(
                self.sheep.get_attr(TIME_TO_REPRODUCE), time_to_repro - 1
            )

    @skip("Learn how to place agent into the model for testing")
    def test_sheep_reproduce(self):
        ...
        # Let sheep reproduce
        reproduce(self.sheep, SHEEP_TIME_TO_REPRO)
    ...
    def test_wolf_reproduce(self):
        ...
        # Let wolf reproduce
        reproduce(self.wolf, WOLF_TIME_TO_REPRO)
